...
import {
  deleteBlogCategory,
  geBlogCategoryDetails,
  updateBlogCategory,
} from "@/api/admin/blog-categories/blog-categories";
...
export const Details = () => {
  ...
  const { data: category, isLoading, mutate } = geBlogCategoryDetails(id);
  ...
  const onSubmit: SubmitHandler<FieldValues> = async data => {
    ...
    try {
      const data = await updateBlogCategory(formData, category.id);
      if (data.error) {
          ...
          <div className="w-full flex justify-between flex-wrap gap-[8px] items-end">
            <div className="block">
              ...
              <span className="block font-meduim font-heading text-[16px] text-gray-600 mt-[8px]">
                Blogs:{" "}
                <span className="text-[#1363DF]">{category.n_blogs}</span>
              </span>
            </div>
