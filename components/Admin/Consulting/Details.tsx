...
import { deleteConsulting, getConsultingDetails } from "@/api/admin/consulting/consulting";
export const Details = () => {
  const params = useParams();
  const id = params.id;

  const { data, isLoading, isError } = getConsultingDetails(id);
  ...
  if (!data) {
    return <p className="text-base">consulting not be found</p>;
  }
  ...
  const handleDeleteClick = async () => {
    ...
    try {
      await deleteConsulting(data.id);
      router.push("/admin/consultings");
    } catch (error) {
      setMessage({
        type: "error",
              ...
              <div className="block">
                ...
                <div className="flex flex-wrap gap-[10px]">
                  <span className="block font-normal font-heading text-[16px] text-gray-600">
                    Phone: <span className="text-[#1363DF]">{data.whats_number}</span>
                  </span>
                  <span className="block font-normal font-heading text-[16px] text-gray-600">
                    Email: <span className="text-[#1363DF]">{data.email}</span>
                  </span>
                  <span className="block font-normal font-heading text-[16px] text-gray-600">
                    Type: <span className="text-[#1363DF]">{data.type}</span>
                  </span>
                </div>
